"""Simple pattern display component for logging morse code patterns."""
...
class PatternDisplay(ConfigurableBase):
    """Simple pattern display for logging morse code patterns.

    Basic visualization component that logs morse pattern events to a time-series graph.
    Provides minimal pattern visualization functionality compared to the full
    GraphicsDisplay dashboard.
    ...
    def __init__(self, cfg_mgr: AwesomeConfigManager, overrides=None):
        ...
        self.logger.info(
            "PatternDisplay initialized: enabled=%s, backend=%s, using TimeSeriesGraph API",
            self.enabled,
            self.backend_type,
        )
    ...
    def _load_config_values(self) -> None:
        ...
        self.update_rate_hz = self._cfg_section.get_int(GraphicsKey.REFRESH_RATE_FPS)  # Convert to Hz equivalent
        self.buffer_size = self._cfg_section.get_double(GraphicsKey.BUFFER_SIZE_SEC)  # Use time-based buffer size
    ...
    def _on_reconfiguration(self) -> None:
        ...
        self._initialize_time_series_graph()
        self.logger.info("PatternDisplay reconfigured: enabled=%s, backend=%s", self.enabled, self.backend_type)
    ...
    def start_display(self) -> None:
        """Start the pattern display (no-op for simple logging)."""
        self.logger.info("Pattern display started - patterns will be logged as they occur")

    def stop_display(self) -> None:
        """Stop the pattern display (no-op for simple logging)."""
        self.logger.info("Pattern display stopped")
