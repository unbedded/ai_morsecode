...
class CodeQualityPipeline:
    ...
    def run_tests(self, coverage: bool = False, verbose: bool = False) -> bool:
        ...
        if coverage:
            cmd.extend(["--cov=src", "--cov-report=term", "--cov-report=html:htmlcov"])
    ...
    def run_pipeline(
        self, fix: bool = False, coverage: bool = False, verbose: bool = False, report: bool = False
    ) -> bool:
...
Examples:
  ...
  python .claude/tools/code_quality.py --fix --report --coverage
    ...
    parser.add_argument("--report", action="store_true", help="Generate additional reports")
